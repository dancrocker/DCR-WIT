...
ratings <- dbReadTable(pool, Id(schema = schema, table = "tblRatings"))
...
cols <- dbListFields(pool, schema_name = schema, name = "tblTribFieldParameters")
...
Uniq <- dbGetQuery(pool, glue("SELECT UniqueID, ID FROM {database}.{schema}.{ImportTable}"))
...
DateTimesMatch <- dbGetQuery(pool, glue("SELECT DateTimeET, Location FROM {database}.{schema}.{ImportTable} WHERE Parameter IN ('Turbidity NTU','Staff Gauge Height')"))
...
### IDs ####
setIDs <- function(){
  query.wq <- dbGetQuery(pool, glue("SELECT max(ID) FROM {database}.{schema}.{ImportTable}"))
  # Get current max ID
  if(is.na(query.wq)) {
    query.wq <- 0
  } else {
...
setFlagIDs <- function(){
  ...
  if(class(df.flags) == "data.frame"){
    query.flags <- dbGetQuery(pool, glue("SELECT max(ID) FROM {database}.{schema}.{ImportFlagTable}"))
    # Get current max ID
    ...
    flag.col.order.wq <- dbListFields(pool, schema_name = schema, name = ImportFlagTable)
