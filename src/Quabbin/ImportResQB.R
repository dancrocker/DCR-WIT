...
#        Dan Crocker. This script will process/import Quabbin Res Field Parameter Data
#        to database
...
#!# Drop unnecesary columns from DF
  df.wq <- df.wq %>% select(-c(DATE, TIME, CablePower.V, pH.mV))

#!# Change our parameter names to match DB parameter abbreviations
  colnames(df.wq) <- c("Depth_M","TWA-C","LDOd","LDOs","pH","SPCD","Chl","BGA-PC","Site","SampleDateTime")
...
# UniqueID
  ...
  df.wq$UniqueID <- paste(df.wq$Site,  df.wq$Depth_M, format(df.wq$SampleDateTime, format = "%Y-%m-%d %H:%M"), df.wq$Parameter, sep = "_")
...
# Reorder remaining columns to match the database table exactly
  df.wq <- df.wq[, c(11, 2:4, 1, 5, 12, 6:10)]
